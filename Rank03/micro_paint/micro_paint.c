...
int ft_ret(float x, float y, t_shape *shape)
{
    if (x < shape->x || y < shape->y || (shape->y + shape->height) < y || (shape->x + shape->width) < x)
        return 0;
    if (x - shape->x < 1.00000000 || y - shape->y < 1.00000000 || (shape->y + shape->height) - y < 1.00000000 || (shape->x + shape->width) - x < 1.0000000)
        return 2;
    return 1;
}
...
void    ft_draw_shape(t_shape *shape, t_zone *zone, char **drawing)
{
    ...
    while (y < zone->height)
    {
        ...
        while (x < zone->width)
        {
            is_it = ft_ret((float)x, (float)y, shape);
...
int ft_draw_shapes(FILE *file, char **drawing, t_zone *zone)
{
    ...
    {
        if (tmp.width < 0.00000000 && tmp.height < 0.00000000 && (tmp.type != 'c' || tmp.type != 'C'))
            return (0);
